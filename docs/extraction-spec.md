...
|---|---|---|---|---|
| LLaVA-Med-1.5 | Mistral-7B-Instruct | 32 | **L0–L4** (frozen 2026-05-31 by 50-case pilot, mean IoU 0.038, robust to prompt variation). | Same early-layer IoU pattern as MedGemma, but **signal magnitude is much weaker**. Against the 24×24 uniform-random baseline: AUC clears p95 (0.66 vs 0.62), but CC/NSS/IoU all fall below p95. Prompt-sensitivity check with a 50-token naturalistic Q&A template (vs original 12-token "There is X visible…") gives metrics within noise (max \|delta\| = 0.012). Finding is robust to prompt structure. Paper bullet: *"LLaVA-Med's attention shows a marginal ordinal preference for bbox regions but no statistically significant shape-correlation with finding locations, regardless of prompt phrasing."* Used the [chaoyinshe HF-format conversion](https://huggingface.co/chaoyinshe/llava-med-v1.5-mistral-7b-hf) because the original microsoft/ checkpoint's `llava_mistral` model_type doesn't deserialize cleanly in transformers v5. See `data/pilot/llavamed/{layer_window,random_baseline}.parquet` + `data/pilot/llavamed_naturalistic/sensitivity_summary.md`. |
